...
fn main() -> Result<()> {
    ...
    match matches.subcommand() {
        ...
        //For now these options are combined.
            ...
            //let sector_size_kibs = value_t!(m, "size", usize).expect("could not convert `size` CLI argument to `usize`");
            let sector_size = Byte::from_str(value_t!(m, "size", String)?)?.get_bytes() as usize;

    // let preserve_cache = true;
    // let skip_precommit_phase1 = false;
    // let skip_precommit_phase2 = false;
