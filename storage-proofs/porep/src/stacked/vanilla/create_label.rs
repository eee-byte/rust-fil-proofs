...
use super::{cache::ParentCache, graph::{StackedBucketGraph, read_node}};
...
/*
pub fn create_label_exp_mmp<H: Hasher>(
    graph: &StackedBucketGraph<H>,
    replica_id: &H::Domain,
    exp_parents_data: &[u8],
    layer_labels: &mut [u8],
    ...
    let hash = if node > 0 {
        ...
        //graph.copy_parents_data_exp(node as u32, &*layer_labels, exp_parents_data, hasher, cache)?
        graph.copy_parents_data_mmap_exp(base_data, exp_data, hasher)
    } else {
        hasher.finish()
    };
